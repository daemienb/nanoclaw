...
async function runQuery(
  ...
  resumeAt?: string,
): Promise<{ newSessionId?: string; lastAssistantUuid?: string; closedDuringQuery: boolean; aborted: boolean }> {  const stream = new MessageStream();
  stream.push(prompt);
  ...
  // Determine model: prefer CLAUDE_CODE_USE_MODEL, then CLAUDE_MODEL, then SDK default
  const modelOverride =
    process.env.CLAUDE_CODE_USE_MODEL ||
